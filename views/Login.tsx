...
import { Lock, User as UserIcon, ShieldAlert, Loader2, UserPlus, ArrowRight, ArrowLeft } from 'lucide-react';
    ...
    try {
      if (isRegistering) {
        ...
        await dataService.createUser({
          ...
          password,
          role: UserRole.ADMIN // Primeiro usuário criado via tela costuma ser Admin
        });
        
        setIsRegistering(false);
        setError('Conta criada com sucesso! Agora você pode acessar.');
      } else {
        const user = await dataService.signIn(username.toLowerCase().trim(), password);
        onLogin(user);
      }
    } catch (err: any) {
      ...
      if (err.message.includes('User already registered')) {
        ...
        setError('E-mail não confirmado. Verifique as configurações no Supabase.');
      } else {
        setError(err.message || 'Erro de conexão com o Supabase. Verifique sua rede.');
      }
    } finally {
      setLoading(false);
    }
      ...
      <div className="bg-white rounded-[40px] shadow-2xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in duration-500">
        <div className="bg-slate-900 p-10 text-center relative overflow-hidden">
          ...
          <p className="text-blue-400 text-[9px] font-black uppercase tracking-[0.2em] mt-2">
            {isRegistering ? 'Cadastro de Colaborador' : 'Plataforma de Performance'}
          </p>
        </div>
        
        <form onSubmit={handleSubmit} className="p-10 space-y-5">
          {error && (
            <div className={`p-4 rounded-2xl text-xs border font-bold flex items-center gap-3 animate-in shake ${error.includes('sucesso') ? 'bg-green-50 text-green-700 border-green-100' : 'bg-red-50 text-red-700 border-red-100'}`}>
              <ShieldAlert size={18} />
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full py-5 bg-slate-900 hover:bg-slate-800 text-white font-black rounded-[28px] shadow-2xl shadow-blue-900/40 transition-all active:scale-95 uppercase tracking-widest text-[10px] flex items-center justify-center gap-3 group"
          >
            {loading ? (
              <Loader2 className="animate-spin" />
            ) : isRegistering ? (
              <>Criar Acesso <UserPlus size={16} /></>
            ) : (
        ...
        <div className="p-6 bg-slate-50 text-center text-[9px] text-slate-400 font-black uppercase tracking-widest border-t border-slate-100">
          &copy; Telemarketing Irmãos Dreon &bull; Supabase Cloud Active
        </div>
      </div>
    </div>
  );
